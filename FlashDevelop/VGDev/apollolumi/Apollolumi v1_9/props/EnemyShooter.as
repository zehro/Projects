package props
{
	...
	{	
		...
		public var accuracy:Number = 2;			// 0 is 100% accuracy
		
		private var targetDelay:int = 30;		// only check for new target every second
		
		public function EnemyShooter(_em:EnemyManager, _cg:MovieClip, _xP:int, _yP:int, _hp:Number, _ts:int)
		{
			...
			else if (tgt)
			{
				if (((abs(x) < LIM_X) && (abs(y) < LIM_Y)) && getDist(this, tgt) < range)
				{
					if (cool == 0)
					{
						...
						//trace("Shooter: SHOT!");
						cg.addProjectile(x - 10, y - 5, 9, rotation + getRand(-accuracy, accuracy), "energy", 90, dmg, false);
						cg.addProjectile(x + 10, y - 5, 9, rotation + getRand(-accuracy, accuracy), "energy", 90, dmg, false);
						dY -= Math.sin(degreesToRadians(rotation)) * moveSpd * 10;		// recoil
						dX -= Math.cos(degreesToRadians(rotation)) * moveSpd * 10;
						cool = rof;
					}
					engL.visible = engR.visible = false;
				}
				else
				{
					engL.visible = engR.visible = true;
					forward();
				}
			}
			return hp <= 0;
